...
public class SlotListTestPopulator : MonoBehaviour
{
	...
	private void OnGUI()
	{
		...
		if (list.Model == null)
		{
            ...
            list.MouseClick += new Bitverse.Unity.Gui.MouseClickEventHandler(list_MouseClick);
        }
    ...
    void list_MouseClick(object sender, Bitverse.Unity.Gui.MouseEventArgs e)
    {
        Vector3 position = new Vector2(Input.mousePosition.x, Screen.height - Input.mousePosition.y);
        
        Debug.LogError("TEST mouse click - SlotIndex = " + list.GetSlotIndexAt(position));
    }
